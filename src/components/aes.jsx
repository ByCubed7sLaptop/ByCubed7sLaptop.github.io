...
Aes.newKeyText = async function() {
    ...
    const key = await window.crypto.subtle.generateKey(options, true, [
        "encrypt",
        "decrypt"
    ]);
...
Aes.encrypt = async function(plainText, key) {
    ...
    const codeBuffer = await window.crypto.subtle.encrypt(
        options,
        key,
        plainBytes
    );
...
Base64.text = function(bytes) {
    ...
    for (let i = 0; i < bytes.length; ++i) {
        ...
        if (r === 0) {
            text +=
                Base64.__getSymbol(bytes[i] >> 2) +
                Base64.__getSymbol(
                    ((bytes[i] << 4) & 63) | (bytes[i + 1] >> 4)
                );
        } else if (r === 1) {
            text += Base64.__getSymbol(
                ((bytes[i] << 2) & 63) | (bytes[i + 1] >> 6)
            );
        } else {
...
AesTextCoder.prototype.decode = function(node) {
    Aes.decrypt(this.__get(node), this.key).then(this.__set.bind(this, node));
};

AesTextCoder.prototype.encode = function(node) {
    Aes.encrypt(this.__get(node), this.key).then(this.__set.bind(this, node));
};

AesTextCoder.prototype.__get = function(node) {
    return node.firstChild.nodeValue;
};

AesTextCoder.prototype.__set = function(node, value) {
    node.firstChild.nodeValue = value;
};

import React, {useRef, useEffect} from "react";

export default function AesC({k, decode, encode, input, children, ...rest}) {
    //Aes.newKeyText().then((k) => { console.log('k: ' + k); });
    ...
    const onGetKey = key => {
        ...
        if (encode) coder.encode(ref.current);
        else if (decode) coder.decode(ref.current);
    };

    useEffect(
        () => {
            if ("subtle" in window.crypto) {
                Aes.getKey(k).then(onGetKey);
            } else {
                console.warn(
                    "AES encryption is only available in secure contexts (such as over https or localhost)"
                );
            }
        },
        [k, decode, encode]
